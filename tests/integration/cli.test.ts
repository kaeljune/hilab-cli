...
integrationDescribe("CLI Integration Tests", () => {
	...
	beforeAll(() => {
		...
		if (!releaseVersion) {
			try {
				const output = execSync(`node ${cliPath} versions --kit coding --limit 1`, {
					encoding: "utf-8",
					stdio: "pipe",
				});
	...
	describe("hi new command", () => {
		test("should create new project in specified directory", async () => {
			const projectDir = join(testDir, "test-hi-new");

			try {
				// Run hi new command with --kit, --force, and --version flags for non-interactive mode
				execSync(
					`node ${cliPath} new --dir ${projectDir} --kit coding --force --release ${releaseVersion}`,
					{
						cwd: testDir,
						stdio: "pipe",
		...
		test("should not overwrite existing project without confirmation", async () => {
			...
			try {
				...
				execSync(`node ${cliPath} new --dir ${projectDir} --kit coding`, {
					cwd: testDir,
					stdio: "pipe",
					timeout: 5000,
				});
