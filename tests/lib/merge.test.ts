...
describe("FileMerger", () => {
	...
	describe("two-tier protection system", () => {
		...
		describe("Tier 2: User config files (USER_CONFIG_PATTERNS)", () => {
			...
			test("should copy .hi.json on first install", async () => {
				await writeFile(join(testSourceDir, "normal.txt"), "normal");
				await writeFile(
					join(testSourceDir, ".hi.json"),
					JSON.stringify({ paths: { docs: "docs", plans: "plans" } }, null, 2),
				);

				await merger.merge(testSourceDir, testDestDir, true);

				expect(existsSync(join(testDestDir, ".hi.json"))).toBe(true);
			});

			test("should preserve existing .hi.json on update (locale settings)", async () => {
				// Create existing .hi.json with user's locale settings in destination
				const existingConfig = {
					locale: { thinkingLanguage: "vi" },
					paths: { docs: "my-docs", plans: "my-plans" },
				};
				await writeFile(join(testDestDir, ".hi.json"), JSON.stringify(existingConfig, null, 2));

				// Create new .hi.json in source (from kit template)
				await writeFile(
					join(testSourceDir, ".hi.json"),
					JSON.stringify({ paths: { docs: "docs", plans: "plans" } }, null, 2),
				);
				...
				// Verify .hi.json was NOT overwritten - user's locale preserved
				const content = await Bun.file(join(testDestDir, ".hi.json")).text();
				const parsed = JSON.parse(content);
