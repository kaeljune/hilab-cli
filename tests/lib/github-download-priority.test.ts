...
describe("GitHubClient - Asset Download Priority", () => {
	describe("getDownloadableAsset", () => {
			...
			const release: GitHubRelease = {
				...
				assets: [
					...
					},
					{
						...
						browser_download_url: "https://github.com/test/hilab-package.zip",
						size: 2048,
		...
		test("should prioritize HiLab Marketing Package zip file", () => {
			const release: GitHubRelease = {
				id: 1,
				tag_name: "v1.0.0",
				name: "Release 1.0.0",
				...
				assets: [
					...
					{
						id: 2,
						name: "HiLab-Marketing-Package.zip",
						url: "https://api.github.com/repos/test/repo/releases/assets/2",
			...
			expect(result.type).toBe("asset");
			expect(result.name).toBe("HiLab-Marketing-Package.zip");
			expect(result.url).toBe("https://api.github.com/repos/test/repo/releases/assets/2");
		});

		test("should match HiLab package case-insensitively", () => {
			const release: GitHubRelease = {
				id: 1,
				tag_name: "v1.0.0",
				name: "Release 1.0.0",
		...
		test("should prioritize HiLab package over other archives", () => {
			const release: GitHubRelease = {
				id: 1,
				tag_name: "v1.0.0",
				name: "Release 1.0.0",
			...
			// Should pick the HiLab package even though it's listed last
			expect(result.type).toBe("asset");
			expect(result.name).toBe("HiLab-Engineer-Package.zip");
		...
		test("should handle assets with variations in naming", () => {
			const release: GitHubRelease = {
				...
				assets: [
					{
						id: 1,
						name: "hilab_marketing_package.zip",
						url: "https://api.github.com/repos/test/repo/releases/assets/1",
						browser_download_url: "https://github.com/test/package.zip",
						size: 2000,
						content_type: "application/zip",
			...
			expect(result.type).toBe("asset");
			expect(result.name).toBe("hilab_marketing_package.zip");
		});
		...
		test("should prioritize HiLab package over source code archives", () => {
			const release: GitHubRelease = {
				id: 1,
				tag_name: "v1.4.0",
				name: "Release 1.4.0",
