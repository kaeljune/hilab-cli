...
describe("Doctor Command", () => {
	...
	beforeEach(async () => {
		...
		const mockMetadata = {
			version: "1.0.0",
			name: "test-hilab",
			description: "Test HiLab for unit testing",
		};
		await writeFile(
	...
	describe("getHiLabSetup", () => {
		test("should detect project setup correctly", async () => {
			const setup = await getHiLabSetup(testDir);
			...
			expect(setup.project.metadata?.name).toBe("test-hilab");
		...
		test("should return empty setup when no .claude directory exists", async () => {
			...
			try {
				const setup = await getHiLabSetup(emptyDir);
