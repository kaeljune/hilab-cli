...
const PROJECT_CONFIG_FILE = ".hi.json";
...
export class ConfigManager {
	...
	/**
	 * Load project-level config from .claude/.hi.json (local) or .hi.json (global).
	 ...
	 * @param global - If true, load from projectDir/.hi.json (treats projectDir as ~/.claude).
	 *                 If false, load from projectDir/.claude/.hi.json (default).
	...
	/**
	 * Save project-level config to .claude/.hi.json (local) or .hi.json (global).
	 ...
	 * @param global - If true, save to projectDir/.hi.json (treats projectDir as ~/.claude).
	 *                 If false, save to projectDir/.claude/.hi.json (default).
	 */
	 ...
	 * 2. Project config (.claude/.hi.json or ~/.claude/.hi.json in global mode)
	 * 3. Global config (~/.hilab/config.json folders section)
	 * 4. Defaults (docs, plans)
	 * @param projectDir - The project directory
	 * @param cliOptions - CLI options for docs and plans directories
	 * @param global - If true, load project config from projectDir/.hi.json (for global mode)
	 */
	static async resolveFoldersConfig(
		...
		// Layer 3: Global config from ~/.hilab/config.json
	 ...
	 * @param projectDir - The project directory. In global mode, this should be ~/.claude.
	 *                     In local mode, this is the project root directory.
	 * @param global - If true, check projectDir/.hi.json (treats projectDir as ~/.claude).
	 *                 If false, check projectDir/.claude/.hi.json (default).
	...
	/**
	 * Migrate .hi.json from nested location to correct location in global mode.
	 * This fixes the bug where .hi.json was incorrectly created at ~/.claude/.claude/.hi.json
	 * instead of ~/.claude/.hi.json
	 *
	...
	static async migrateNestedConfig(globalDir: string): Promise<boolean> {
		...
		if (existsSync(incorrectPath)) {
			try {
				logger.info("Migrating .hi.json from nested location to correct location...");
