...
export async function detectMetadataFormat(claudeDir: string): Promise<MetadataFormatDetection> {
	...
	try {
		...
		if (parsed.name || parsed.version || parsed.files) {
			...
			// Legacy "engineer" name is auto-remapped to current "coding" kit.
			...
			if (/\b(engineer|coding)\b/i.test(nameToCheck)) {
				...
				// Default to coding for unnamed legacy installs
				detectedKit = "coding";
			}
...
export async function migrateToMultiKit(claudeDir: string): Promise<MetadataMigrationResult> {
	...
	const legacyKit = detection.detectedKit || "coding";
	...
	try {
		...
		// breaking existing tools/scripts that may read them. On next `hi init/update`, these
...
export function getInstalledKits(metadata: Metadata): KitType[] {
	...
	// Default to coding for legacy installs with version but no identifiable name
	if (metadata.version) {
		return ["coding"];
	}
