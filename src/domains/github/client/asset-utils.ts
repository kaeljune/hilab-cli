 ...
 * Priority:
 * 1. "HiLab Engineer Package" or "HiLab Marketing Package" zip file
...
export function getDownloadableAsset(release: GitHubRelease): {
	...
	// First priority: Look for official HiLab package assets
	const packageAsset = release.assets.find((a) => {
		const nameLower = a.name.toLowerCase();
		return (
			nameLower.includes("hilab") && nameLower.includes("package") && nameLower.endsWith(".zip")
		);
	});

	if (packageAsset) {
		logger.debug(`Selected HiLab package asset: ${packageAsset.name}`);
		return {
			type: "asset",
	...
	logger.debug("No HiLab package asset found, checking for other custom assets...");
