...
describe("InstallWizard", () => {
	...
	describe("Rendering", () => {
		...
		it("shows step 1 content initially", () => {
			render(<InstallWizard {...defaultProps} />);
			expect(screen.getByText(/Selected: HiLab Engineer/)).toBeInTheDocument();
		});
	});
	...
	describe("Callbacks", () => {
		...
		it("calls onKitSelect when switch kit button is clicked", () => {
			...
			const switchButton = screen.getByText(/Switch to HiLab Marketing/);
	...
	describe("Kit Selection", () => {
		it("shows Marketing kit switch option when Engineer selected", () => {
			...
			expect(screen.getByText(/Switch to HiLab Marketing/)).toBeInTheDocument();
		});
		...
		it("shows Engineer kit switch option when Marketing selected", () => {
			...
			expect(screen.getByText(/Switch to HiLab Engineer/)).toBeInTheDocument();
		});
