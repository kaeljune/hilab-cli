...
describe("maybeShowConfigUpdateNotification", () => {
	...
	it("returns false when no updates available (cached)", async () => {
		...
		await writeFile(join(cacheDir, "coding-config-update-cache.json"), JSON.stringify(cache));
