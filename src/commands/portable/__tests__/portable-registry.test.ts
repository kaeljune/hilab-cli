 ...
 * Note: These tests use the real ~/.hilab/ directory
 */
...
const REGISTRY_PATH = join(homedir(), ".hilab", "portable-registry.json");
const MIGRATION_LOCK_PATH = join(homedir(), ".hilab", ".migration.lock");
...
afterEach(async () => {
	...
	if (hadMigrationLock) {
		await mkdir(join(homedir(), ".hilab"), { recursive: true });
...
describe("v2.0 to v3.0 migration", () => {
	...
	test("reads target file for targetChecksum during migration", async () => {
		const targetPath = join(homedir(), ".hilab", "test-target-file.md");
	...
	test("preserves all v2.0 fields during migration", async () => {
			...
			installations: [
				{
					...
					sourcePath: "/hilab/commands/my-command.md",
					cliVersion: "1.5.0",
				},
			],
		};
		...
		expect(inst.sourcePath).toBe("/hilab/commands/my-command.md");
