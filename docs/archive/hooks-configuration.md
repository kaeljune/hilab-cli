...
This document explains the hooks configured in `.claude/settings.json` for the HiLab CLI project.
...
**Purpose:** Automatically check for HiLab updates when a new session starts.
...
- Compares current HiLab version with latest GitHub release
- Displays notification if update available
- Caches results for 7 days to minimize API calls
- Cache location: `~/.hilab/cache/version-check.json`
...
1. **Check cache age:** Cache may be fresh (<7 days old)
   ```bash
   cat ~/.hilab/cache/version-check.json
   ```

2. **Clear cache to force refresh:**
   ```bash
   rm ~/.hilab/cache/version-check.json
   ```
